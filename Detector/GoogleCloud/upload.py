...
class GoogleServices:
    ...
    # To only be used to download config .yaml files.
    def downloadConfig(self, id):
        configFile = self.drive.CreateFile({'id' : id})
        content = yaml.safe_load(configFile.GetContentString(mimetype='text/yaml'))
        # print(recipients)
        return content

# Test script for uploading log file.
# gdSession = GoogleServices('testSession', 'testing_folder/GoogleCloud/service-secrets.json') # note, datetime will automatically be appended to the folder name.
